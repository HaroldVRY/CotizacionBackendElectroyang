...
const MaestroModel_1 = require("../models/MaestroModel");
const express_validator_1 = require("express-validator");
const maestroModel = new MaestroModel_1.MaestroModel();
class MaestroController {
    // ===== CABECERA =====
    async getAllCabeceras(req, res) {
        try {
    ...
    async getDetallesByCabecera(req, res) {
        try {
    ...
    async toggleDetalleEstado(req, res) {
        try {
            const { id } = req.params;
            const existingDetalle = await maestroModel.getDetalleById(parseInt(id));
            if (!existingDetalle) {
